...
function onSearch(e) {
    if (e.target.value !== '') {
        ...
        fetchCountries(fieldValue).then(data => {
            if (data.length > 10) {
...
function createMarkup(arr) {
        const markup = arr.map(el => {
            const { name: { official }, capital, population, flags: { svg }, languages } = el;  
        ...
        <li>
          <h3>Languages:</h3>
          <span class="content-span">${Object.values(languages).join(', ')}</span>
        </li>
      </ul>`
            }
            return `<li class="country-item">
